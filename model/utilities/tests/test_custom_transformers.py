...
@pytest.mark.parametrize(
    "input_data, target_string, expected_output",
    [
        (
            pd.DataFrame({"text_column": ["apple", "banana"]}),
            "an",
            pd.DataFrame({"text_column": [0.0, 1.0]}),
        ),
        (
            pd.DataFrame({"text_column": ["apple", "banana"]}),
            "ch",
            pd.DataFrame({"text_column": [0.0, 0.0]}),
        ),
        (
            pd.DataFrame({"text_column": ["apple", "banana"]}),
            "kiwi",
...
def test_string_contains_transformer_without_nans(
    input_data, target_string, expected_output
):
    """
...
@pytest.mark.parametrize(
    ...
    [
        (
            pd.DataFrame({"text_column": ["banana", np.nan]}),
            "e",
            pd.DataFrame({"text_column": [0.0, np.nan]}),
        ),
        (
            pd.DataFrame({"text_column": ["banana", np.nan]}),
            "na",
            pd.DataFrame({"text_column": [1.0, np.nan]}),
        ),
        (
            pd.DataFrame({"text_column": [np.nan, "kiwis"]}),
            "kiwi",
            pd.DataFrame({"text_column": [np.nan, 1.0]}),
        ),
        (
            pd.DataFrame({"text_column": [np.nan]}),
            "ga",
            pd.DataFrame({"text_column": [np.nan]}),
        ),
    ],
)
def test_string_contains_transformer_with_nans(
    input_data, target_string, expected_output
):
    """
    Verify that the transform method produces the expected result without nans.
...
@pytest.mark.parametrize(
    "target_string, expected_feature_names_out",
    [
        (
            "apple",
            ["text_column_1_apple", "text_column_2_apple", "text_column_3_apple"],
        ),
        (
            "banana",
            ["text_column_1_banana", "text_column_2_banana", "text_column_3_banana"],
        ),
        (
            "cherry",
            ["text_column_1_cherry", "text_column_2_cherry", "text_column_3_cherry"],
        ),
    ],
)
